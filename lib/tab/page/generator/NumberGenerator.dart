...
import 'package:lottery_kr/tab/page/auth/service/NumberGenerateService.dart';
import 'dart:math';
...
class NumberGenerator extends StatefulWidget {
  final String lotto;
  const NumberGenerator({super.key, required this.lotto});
...
class _NumberGeneratorState extends State<NumberGenerator> {
  ...
  Future _saveData() async {
    try {
      ...
      await prefs.setStringList(widget.lotto, stringList);
    } catch (e) {
  ...
  void _loadData() async {
    try {
      final SharedPreferences prefs = await SharedPreferences.getInstance();
      List<String> stringList = prefs.getStringList(widget.lotto) ?? [];
      setState(() {
  ...
  void initState() {
    ...
    Future.delayed(Duration(seconds: 0)).then((value) async {
      ...
      if (isOnWifi) {
        List<List<dynamic>> numbers = await lotteryService.getMostWorstShownNumbers(widget.lotto);
  ...
  Widget build(BuildContext context) {
    return (isDataLoading || isNumbersLoading || isLimitLoading) ? const Center(child: CircularProgressIndicator()) : Scaffold(
        ...
        Expanded(
          child: TextFieldTags<String>(
            ...
            validator: (String tag) {
              ...
              String res = numberGenerateService.checkNumberCondition(widget.lotto, _manualTagController, tag);
              if (res != "") return res;
              return null;
            },
            inputFieldBuilder: (context, inputFieldValues) {
