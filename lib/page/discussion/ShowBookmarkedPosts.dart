...
class ShowBookmarkedPosts extends StatefulWidget {
  const ShowBookmarkedPosts({super.key});
...
class _ShowBookmarkedPostsState extends State<ShowBookmarkedPosts> {
  ...
  Future<void> initializeData() async {
    try {
      var likedPostIds = await userService.getBookmarkedPosts(uid);
      ...
      setState(() {
        bookmarkedPosts = discussions; // Assuming likedPosts is declared somewhere in your class
    ...
    body: SafeArea(
      ...
      child: Column(
        ...
        children: [
          ...
          Expanded(
            child: bookmarkedPosts.isNotEmpty ? ListView.builder(
              controller: _scrollController,
              itemCount: bookmarkedPosts.length,
              itemBuilder: (context, index) {
                return buildDiscussionCard(index);
              },
            ) : Center(child: Text("noBookmarked".tr(), style: TextStyle(color: Colors.white))),
          ),
        ],
      ),
    ),
...
Widget buildTopNavigation(BuildContext context) {
  return Row(
    ...
    children: [
      ...
      GestureDetector(
        onTap: () => scrollToTop(),
        child: Text("bookmarkedPosts".tr(), style: TextStyle(color: Colors.white, fontSize: 20, fontStyle: FontStyle.italic)),
      ),
      Padding(
        padding: const EdgeInsets.all(16.0),
        child: Builder(
...
void scrollToTop() {
  ...
  Widget buildDiscussionCard(int index) {
    try {
      if (bookmarkedPosts[index]["lottery"] == category || category == "All") {
        var discussion = bookmarkedPosts[index];
